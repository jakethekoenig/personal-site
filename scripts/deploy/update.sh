...
aws s3 sync "$LIVE/asset/icon" s3://ja3k.com/asset/icon --size-only
aws s3 sync "$LIVE/" s3://ja3k.com/ --exclude "css*" --exclude "js*" --exclude "asset*" --exclude "*.xml" --exclude "*.txt" --content-type "text/html" --size-only
aws s3 cp "$LIVE/" s3://ja3k.com/ --recursive --exclude "*" --include "*.xml" --include "*.txt"

aws cloudfront create-invalidation --distribution-id E3RFZ3RTME1070 --paths "/*"
